...
def send_register_request():
    ...
    get_rec(gw)
    gw.close()
def L2_RelayMsgConnectionResponse(gw):
    rec = gw.recv(4)
    #print "rec is" + repr(rec)
    version , msg_type, length = struct.unpack('!2cH',rec)
    #print "version, msg_type, length" + repr(version) , repr(msg_type), length
...
def get_rec(gw):
    ...
    while(1):
        msg_type, res = L2_RelayMsgConnectionResponse(gw)
        #print "get_rec msg_type is"+ repr(msg_type) +"~~~~~res is "+ repr(res)
