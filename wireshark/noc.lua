do
	...
	local function noc_dissector(buf, pkt, root)
        ...
        if msg_type == 0 then -- RelayQueryConnectionRequest
        	t:add(f_noc_target_host_id, buf(4,4)) --test add
        	t:add(f_noc_host_id, buf(8,4))                
        	pkt.cols.info = "RelayQueryConnectionRequest"
